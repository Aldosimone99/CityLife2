...
var DevtoolsExtension = class _DevtoolsExtension {
  ...
  createActionStreams() {
    const changes$ = this.createChangesObservable().pipe((0, import_operators.share)());
    const start$ = changes$.pipe((0, import_operators.filter)((change) => change.type === ExtensionActionTypes.START));
    const stop$ = changes$.pipe((0, import_operators.filter)((change) => change.type === ExtensionActionTypes.STOP));
    const liftedActions$ = changes$.pipe((0, import_operators.filter)((change) => change.type === ExtensionActionTypes.DISPATCH), (0, import_operators.map)((change) => this.unwrapAction(change.payload)), (0, import_operators.concatMap)((action) => {
      if (action.type === IMPORT_STATE) {
        return this.dispatcher.pipe((0, import_operators.filter)((action2) => action2.type === UPDATE), (0, import_operators.timeout)(1e3), (0, import_operators.debounceTime)(1e3), (0, import_operators.map)(() => action), (0, import_operators.catchError)(() => (0, import_rxjs.of)(action)), (0, import_operators.take)(1));
      } else {
        return (0, import_rxjs.of)(action);
      }
    }));
    const actions$ = changes$.pipe((0, import_operators.filter)((change) => change.type === ExtensionActionTypes.ACTION), (0, import_operators.map)((change) => this.unwrapAction(change.payload)));
    const actionsUntilStop$ = actions$.pipe((0, import_operators.takeUntil)(stop$));
    const liftedUntilStop$ = liftedActions$.pipe((0, import_operators.takeUntil)(stop$));
    this.start$ = start$.pipe((0, import_operators.takeUntil)(stop$));
    this.actions$ = this.start$.pipe((0, import_operators.switchMap)(() => actionsUntilStop$));
    this.liftedActions$ = this.start$.pipe((0, import_operators.switchMap)(() => liftedUntilStop$));
  }
  unwrapAction(action) {
...
var StoreDevtools = class _StoreDevtools {
  constructor(dispatcher, actions$, reducers$, extension, scannedActions, errorHandler, initialState, config) {
    ...
    const liftedAction$ = (0, import_rxjs.merge)((0, import_rxjs.merge)(actions$.asObservable().pipe((0, import_operators.skip)(1)), extension.actions$).pipe((0, import_operators.map)(liftAction)), dispatcher, extension.liftedActions$).pipe((0, import_operators.observeOn)(import_rxjs.queueScheduler));
    const liftedReducer$ = reducers$.pipe((0, import_operators.map)(liftReducer));
    const zoneConfig = injectZoneConfig(config.connectInZone);
    const liftedStateSubject = new import_rxjs.ReplaySubject(1);
    this.liftedStateSubscription = liftedAction$.pipe(
      (0, import_operators.withLatestFrom)(liftedReducer$),
    ...
    const state$ = liftedState$.pipe((0, import_operators.map)(unliftState));
...
function emitInZone({
  ...
  return (source) => connectInZone ? new import_rxjs.Observable((subscriber) => source.subscribe({
