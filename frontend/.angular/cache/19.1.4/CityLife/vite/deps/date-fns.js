...
function clamp(date, interval2, options) {
  const [date_, start, end] = normalizeDates(options?.in, date, interval2.start, interval2.end);
...
function eachDayOfInterval(interval2, options) {
  ...
  } = normalizeInterval(options?.in, interval2);
...
function endOfISOWeek(date, options) {
  return endOfWeek(date, __spreadProps(__spreadValues({}, options), {
    weekStartsOn: 1
  }));
}
...
function isSameISOWeek(laterDate, earlierDate, options) {
  return isSameWeek(laterDate, earlierDate, __spreadProps(__spreadValues({}, options), {
    weekStartsOn: 1
  }));
}
...
function isWithinInterval(date, interval2, options) {
  ...
  const [startTime, endTime] = [+toDate(interval2.start, options?.in), +toDate(interval2.end, options?.in)].sort((a, b) => a - b);
...
function lastDayOfISOWeek(date, options) {
  return lastDayOfWeek(date, __spreadProps(__spreadValues({}, options), {
    weekStartsOn: 1
  }));
}
